...
if($searchtype=='short'){
	...
	//if($term!=''){
		...
		$pattern="SELECT title,url,article_page_image_path,summary_html,publish_start_date FROM article WHERE title LIKE '%".$term."%' OR summary_html LIKE '%".$term."%' OR author_name LIKE '%".$term."%' OR agency_name LIKE '%".$term."%' AND publish_start_date <=NOW()  ORDER BY publish_start_date DESC";
...
<div class="adv_search_result">
	...
	if(!isset($_GET['request']) && $_GET['request']==''){$_GET['request'] = 'ALL'; }
	/*for keyword search*/
	if(isset($_GET['request']) && $_GET['request']=='ALL'):
		...
		if($check_archive==''){
			...
			foreach($range as $ranger):
				...
				if($this->archive_db->table_exists($table)){
					$archive_pattern="SELECT title,url,article_page_image_path,summary_html FROM ".$table." WHERE title LIKE '%".$term."%' OR summary_html LIKE '%".$term."%' OR author_name LIKE '%".$term."%' OR agency_name LIKE '%".$term."%' AND publish_start_date <=NOW()";
		...
		if(isset($_GET['archive']) && $_GET['archive']==true && $_GET['year']!=''){
			...
			$pattern_archive="SELECT title,url,article_page_image_path,summary_html,publish_start_date FROM ".$archive_tbl." WHERE title LIKE '%".$term."%' OR summary_html LIKE '%".$term."%' OR author_name LIKE '%".$term."%' OR agency_name LIKE '%".$term."%' AND publish_start_date <=NOW()";
	...
	if(isset($_GET['request']) && $_GET['request']=='MIN'):
		...
		$pattern="SELECT title,url,".$image_path." as article_page_image_path,summary_html,publish_start_date FROM ".$live_db." WHERE ".$type_query." AND publish_start_date <=NOW() ORDER BY publish_start_date DESC";
		...
		if($check_archive==''){
			...
			foreach($range as $ranger):
				...
				//	echo $table;
				//	echo '<br>yes';
				$archive_pattern="SELECT title,url,".$image_path." as article_page_image_path ,summary_html FROM ".$table." WHERE ".$type_query." AND publish_start_date <=NOW() ORDER BY publish_start_date DESC";
	...
	if($tags!=''){
		...
		$tag_query="SELECT title,url," .$image_col. " as article_page_image_path ,summary_html,publish_start_date FROM ".$tag_type." WHERE tags LIKE '%".$tags."%' ORDER BY publish_start_date DESC";
