        ...
        <ul class="nav navbar-nav menus">
			   ...
			   $MainSectionPageURL = base_url(). $url_section_value;												
				 ...
				 if(strtolower($mapping['Sectionname']) != "home")
				 {
						/* if($Section_Count==2){
							...
							echo '<li class="StatesHover" id="tab284"><a class="MenuItem" id="maintabs-284" href="'.base_url('sport/icc-world-cup').'">World Cup</a></li>';
						}  */
						if($mapping['SectionnameInHTML']=='<p>Entertainment</p>'){ $mapping['SectionnameInHTML'] ='<p>Movies</p>'; }	
						if($mapping['SectionnameInHTML']=='<p>Galleries</p>'){ $mapping['SectionnameInHTML'] ='<p>Photos</p>'; }					
          ...
          <li class="<?php  if(!empty( $mapping['sub_section']) && $Section_Details['Sectionname'] != "Magazine") { echo "CitiesHover";  } else { echo "StatesHover";  } ?>" id="tab<?php echo $mapping['Section_id']; ?>"><a class="MenuItem"  id="maintabs-<?php echo $mapping['Section_id']; ?>" <?php if(count($mapping['sub_section']) == 0) { ?> onmouseover="show_main_menu('<?php echo $mapping['Section_id'];?>', 'main')" <?php } else { ?> onmouseover="show_main_menu('<?php echo $mapping['sub_section'][0]['Section_id']; ?>','<?php echo $mapping['Section_id'];?>')"  <?php }?> href="<?php echo $MainSectionPageURL; ?>"><?php echo preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1',$mapping['SectionnameInHTML']); ?></a>
            <?php
						...
						if($mapping['Sectionname'] == "Sport"){
							echo '<li class="StatesHover" id="tab398"><a class="MenuItem" id="maintabs-398" href="'. base_url('good-news') .'">Good News</a></li>';
						}
						if($mapping['Sectionname'] == "Videos"){
							echo '<li class="StatesHover epaper-d" id="tab2841"><a target="_blank" class="MenuItem" id="maintabs-2841" href="https://epaper.newindianexpress.com/" style="color:#fff !important;"><span> SUBSCRIBE TO E-PAPER</span></a></li>';
						}
