		...
		foreach($widget_instance_contents as $get_content)
		{
			...
			if($render_mode == "manual")            // from widgetinstancecontent table    
			{
				if($get_content['custom_image_path'] != '')
				{
					...
					$content_url         = (isset($content_details[0]['url'])) ? $content_details[0]['url'] :'';
			...
			if($original_image_path =="" && $render_mode =="manual")     // from cms || Live table    
				{
					   $original_image_path  = (isset($content_details[0]['ImagePhysicalPath'])) ? $content_details[0]['ImagePhysicalPath'] :'';
					   $imagealt             = (isset($content_details[0]['ImageCaption'])) ? $content_details[0]['ImageCaption'] :'';	
					   $imagetitle           = (isset($content_details[0]['ImageAlt'])) ? $content_details[0]['ImageAlt'] :'';
				}	
