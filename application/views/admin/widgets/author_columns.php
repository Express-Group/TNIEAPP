...
$author_image_id='';
//$perpage				=	15;
if($this->uri->segment(4)=='archive' || $this->uri->segment(3)=='archive'){
	$TotalCount=$this->widget_model->GetAuthorArchiveDetails($author_name,$content['page_param'],2);
	$config['total_rows'] = $TotalCount;
	if(is_numeric($this->uri->segment(3))){
		$config['base_url']=$domain_name.'Author/'.$this->uri->segment(2).'/'.$this->uri->segment(3).'/archive';
	}else{
    ...
    $config['first_url']=$domain_name.'Author/'.$this->uri->segment(2).'/'.$this->uri->segment(3);
...
// newly add for paging end 
	...
	$author_name      = (isset($author_det[0]['AuthorName']) && $author_det[0]['AuthorName']!='') ? $author_det[0]['AuthorName'] : '';
	$ShortBiography   = (isset($author_det[0]['ShortBiography']) && $author_det[0]['ShortBiography']!='') ? $author_det[0]['ShortBiography'] : '';
	$author_image_id  = (isset($author_det[0]['image_id']) && $author_det[0]['image_id']!='') ? $author_det[0]['image_id'] : '';
      ...
      <div class="current all_column">
        <?php
		 ...
		 foreach($columnnist_articles_list as $article_list){ 
			...
			if($this->uri->segment(3)=='archive' || $this->uri->segment(4)=='archive'){
			...
			?>
        <div class="sub_column"> <i class=""></i>
        ...
        <?php } 
		  ...
		  <?php
			$archive='';
			$t = (is_numeric($this->uri->segment(3))) ? $this->uri->segment(4) : $this->uri->segment(3);
			if(($GetArchiveDetails > 0) && $t==""){
				if(is_numeric($this->uri->segment(3))){
					$load_more_url = $domain_name.'author/'.$this->uri->segment(2).'/'.$this->uri->segment(3).'/archive';	
				}else{
