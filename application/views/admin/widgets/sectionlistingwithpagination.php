...
$cacheID = 'sectionlistingwithpagernum_rows'.$sectionid;
if(!$this->memcached_library->get($cacheID) && $this->memcached_library->get($cacheID) == ''){
	...
	$this->memcached_library->add($cacheID,$totalrows);
}else{
	$totalrows = $this->memcached_library->get($cacheID);
}
...
$cacheID1 = 'sectionlistingwithpagernum_rows'.$sectionid.$_GET['per_page'].$config['per_page'];
if(!$this->memcached_library->get($cacheID1) && $this->memcached_library->get($cacheID1) == ''){
	...
	$this->memcached_library->add($cacheID1,$ArticleData);
}else{
	$ArticleData = $this->memcached_library->get($cacheID1);
}
...
foreach($ArticleData as $article){
	...
	echo '<h4 style="font-weight:normal;font-family: Droid Serif !important;">'.$display_title.'</h4>';
