...
if ($content_id != '')
{
	...
	if (count($widget_contents) > 0)
	{
		foreach ($widget_contents as $get_content)
		{
			...
			if ($lastpublishonformat != date('d.m.Y', strtotime($get_content['last_updated_on'])))
			{
				$lastpublishonformat = date('d.m.Y', strtotime($get_content['last_updated_on']));
				$show_simple_tab .= "<date>" . date('d.m.Y', strtotime($get_content['last_updated_on'])) . "</date>";
			}
			$show_simple_tab .= '<i class="fa fa-angle-right"></i><p>' . $display_title . '</p>';
			
			$post_time = $this->widget_model->time2string($get_content['last_updated_on']);
