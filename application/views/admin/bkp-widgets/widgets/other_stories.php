...
if($page_type == 1)
	{
		...
		if($parentsection_id != "")
		{
			...
			$sections            = $this->widget_model->get_section_menudisplay($parentsection_id);
				...
				if(trim($section_name) != 'Cities' && trim($section_name) != 'States' && trim($section_name) != 'Opinions')
				{
					...
					foreach($type1stories as $r => $f)
					{
						...
						if($io != '999999999999999999')
							$widget_instance_contents_array[$r] = $this->widget_model->get_RightSide_OtherStories_Contents_Type1('1', $view_mode, $io, '');
					}
					...
					//print_r($widget_instance_contents_array);
					$i = 1;
				}
				else
				{
					if(trim($section_name) == 'States')
					{
						...
						$city_state          = $this->widget_model->get_section_menudisplay($section_id, $view_mode);
						$max_limit = 1;
					}
