...
if($content_id!='')
{
	...
	{	
		...
		foreach($widget_contents as $get_content)
		{
			...
			$author_url = ($get_seperation_count==1)? $author_url : (($get_seperation_count==2)? $url_array[0]."/".$url_array[1] : $url_array[0]."/".$url_array[1]."/".$url_array[2]);
			$author_pos = strpos($author_url, "columns");
            if ($author_pos === false) {
			$author_new_url = $domain_name.'Author/'.$author_url;
			}else
			{
			$author_new_url = $domain_name.$author_url;
			}
