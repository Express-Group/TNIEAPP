...
$show_simple_tab .='<div class="row">
		...
		foreach($widget_instancemainsection as $get_section)
		{
			...
			$show_simple_tab .= '<li '.$add_active.' id="state'.$get_section['Section_ID'].'">'.$get_section['CustomTitle'].'<i class="fa fa-chevron-right"></i></li>';
			$l++;
		}
		}
		else
		...
		{
			$add_active=($l==1)?'class="relate resp-tab-active"' : '';
			$show_simple_tab .= '<li '.$add_active.' id="state'.$get_section['cdata-categoryId'].'">'.$get_section['cdata-customTitle'].'<i class="fa fa-chevron-right"></i></li>';
			$l++;
		}
		}
		$show_simple_tab .='</ul>
		...
		foreach($widget_instancemainsection as $get_section)
		{
			...
			if($content['RenderingMode'] == "manual"){
				...
				$main_sction_id = $get_section['cdata-categoryId'];
			}
			}
			if($content['RenderingMode'] == "manual"){
...
<div class=" ajaxStatus ajaxFailed noDisplay" style="display: none;">Failed to load the content...</div></div>';
			...
			$show_simple_tab .='<div id="state_content'.$get_section['cdata-categoryId'].'">
			<div class="cssload-container cssload-container-states" id="add_article_process_img'.$get_section['cdata-categoryId'].'" style="display:none;"><figure>
  <img src="'.base_url().'images/FrontEnd/images/loader-Nie.png"><div class="cssload-zenith"></div></figure></div>
<div class=" ajaxStatus ajaxFailed noDisplay" style="display: none;">Failed to load the content...</div></div>';
			}
...
show_states_articles(<?php echo $main_sction_id;?>, 'state<?php echo $main_sction_id;?>');	
...
if(list==accord){
 ...
 itemCount++;
	...
	if ($(this).attr('id')){
		...
		show_states_articles(stateid, id);
		}
      },
	});
...
$('#parentVerticalTab_<?php echo $widget_instance_id; ?> .resp-tab-content[aria-labelledby="hor_1_tab_item-0"]').addClass('resp-tab-content-active');
  $('#vertical-ticker_<?php echo $widget_instance_id; ?>').totemticker({ 
	...
	function show_states_articles(stateid, id){
	$('#add_article_process_img'+stateid).css('display','block');
		 $.ajax({
			url			: '<?php echo base_url(); ?>user/commonwidget/get_states_content',
			method		: 'post',
			data		: { stateid: stateid,widgetinstanceid: '<?php echo $widget_instance_id;?>',mode: '<?php echo $content['mode'];?>', 'rendermode' : '<?php echo $content['RenderingMode'];?>', is_home : '<?php echo $is_home;?>', max_article : '<?php echo $content['show_max_article'];?>', summary_option: '<?php echo $is_summary_required;?>',param : '<?php echo $content['page_param'];?>' },
			beforeSend	: function() {				
