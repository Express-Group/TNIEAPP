...
class Widget_model extends CI_Model
{
  ...
  public function __construct() 
	...
	public function StandOutStatus($ContentID){
		if($ContentID==''){
			return 0;
		}else{
			$Query=$this->live_db->query("SELECT content_id FROM standoutarticlemanager WHERE content_id='".$ContentID."'")->num_rows();
			if($Query==0){
				return 0;
			}else{
				return 1;
...
class Widget_config_class extends Widget_model
{
	...
	public function GetAuthorArchiveDetails($author_name,$SectionID,$type){
		...
		$start_range = (isset($year) && $year!='') ? $year : Date('Y'); 
		...
		foreach($year_range as $year)
		{
			...
			{
				if($type==1){
					$query = $archive_db->query("SELECT content_id FROM ".$tablename." WHERE author_name LIKE '".$author_name."' AND status='P'")->num_rows();
					if($query > 0){
						$result_count=$query;
						break;
					} 
	...
	public function GetContentBasedOnArchive($limit,$author_name,$totalcount){
		...
		foreach($year_range as $year){
			...
			if($archive_db->table_exists($tablename)){
				if($count==$i):
				$query .="SELECT title,url,summary_html,last_updated_on FROM ".$tablename." WHERE author_name LIKE '".$author_name."' AND status='P'";
				else:
				$query .="SELECT title,url,summary_html,last_updated_on FROM ".$tablename." WHERE status='P' AND author_name LIKE '".$author_name."'  UNION ";
				endif;
			}
		$i++;
		}
	...
	public function Get_dynamic_tables(){
		$this->load->database();
		$Query=$this->db->query("SELECT tid,table_name,table_properties,total FROM tablemaster WHERE status='0' AND (table_properties!=NULL OR table_properties!='') ORDER BY listorder ASC");
		if($Query->num_rows() > 0){
			return $Query->result();
		}else{
			return 'NO ROWS';
 ...
 public function get_liveContents_by_sectionId($max_article, $section_id, $view_mode, $lead_story_maximum_Articles , $subsec_leadstory_remdering_mode, $leadstory_contentID, $contentType_id,$is_home)
{
  ...
  public function get_Stories_For_Author_all($view_mode,$authname)
 {
    ...
    $order    = "DESC";
    $start_limt  = "0";
    $length   = (isset($max_article)) ? $max_article : '';
    $check_in   = "";
    $check_out   = "";
    $Search_value  = "";
    $Search_by   = "";
    $Section   = (isset($section_id)) ? $section_id : '';
    $Status   = "P";
    $content_type  = "1";
	
		$article_manager  =  $this->live_db->query('CALL  get_Stories_For_Author_all ("'.$authname.'")');
