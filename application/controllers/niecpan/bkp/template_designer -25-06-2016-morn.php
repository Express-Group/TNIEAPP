...
class Template_designer extends CI_Controller {
	...
	public function save_xml_template()
	{
		...
		if($design_lock_status == 2) ////  Template is unlocked  ////
		{
			...
			if($publish_xml == "publish")
			{
				...
				if($page_type=='1'){
				$file_name		= ($section_id != 10000) ? $section_id."_1.xml" : $section_id."_1.xml"; 
				}else{
				$file_name		= ($section_id != 10000) ? $section_id."_2.xml" : $section_id."_2.xml";  
				}
			}
			else
			{
			$save_path		= FCPATH."uploads/page_layouts/temp/";
				if($page_type=='1'){
				$file_name		= ($section_id != 10000) ? $section_id."_1.xml" : $section_id."_1_".$version_id.".xml"; 
				}else{
				$file_name		= ($section_id != 10000) ? $section_id."_2.xml" : $section_id."_2_".$version_id.".xml";  
				}
			}	
	...
	public function load_template()
	{
		...
		//echo "count: ".count($template_details); 
		//print_r($post); 
		...
		//print_r($template_details); exit;
		$result = array("template_xml" => $template_xml, "is_template_closed_before_save" => $is_template_closed_before_save, "template_lock_status" => $template_lock_status, "template_locked_user" => $template_locked_user, "template_locked_user_name" => $template_locked_user_name, "total_versions" => $total_versions); //is_template_closed_before_save- 1->Yes, 2->No
		header("content-type: application/json"); 
	...
	{		
		$template_xml 	= $this->template_design_model->get_allwidgets('1');	
		header("content-type: application/json");
		echo json_encode($template_xml);
	}
	...
	/*public function dynamic_templates_count()
	{
		$template_details = $this->template_design_model->get_templates_count('1');	
				
		header("content-type: application/json");
		$templateString_json['total_count'] = $template_details['total_templates_count'];
	...
	{		
		...
		//print_r($template_details); exit;
		//$this->load->view("dynamic_templates",$data);
				$j=1;			
				...
				///////  Store the templates in template_master table  //////
				//$this->template_design_model->update_templates($templateString);
				
				//print_r($templateString); exit;
				//return $templateString;
				header("content-type: application/json");
				$templateString_json['templ'] = $templateString;
	...
	public function dynamic_containers()
	{
		...
		//$this->load->view("dynamic_container",$data);
		$i = 1;
		...
		{		
			...
			$container_content[$content_id] = '<div class="container-wrapper section group" data-type="'. $type .'" id="container-'. $id .'">' .			
				'<div class="section group">'.((FPM_ADDPAGEDESIGN) ? '<div class="handle"><span class=" close-container fa fa-times"></span></div>' : '<div class="handle-disabled"></div>').
				'</div>' .				
			...
			///////  Store the templates in template_master table  //////
			//$this->template_design_model->update_containers($container_content);
			
			header("content-type: application/json");
			$templateString_json['templ'] = $container_content;
			echo json_encode($templateString_json);
	}
				...
				foreach(@$value['sub_section'] as $skey => $svalue)
				{
					if($svalue['section_allowed_for_hosting'] != 0)
					{
						...
						{	
							...
							{								
								$special_section[] = array('categoryId' => $spl_value['Section_id'], 'categoryName' => $spl_value['Sectionname'], "section_allowed_for_hosting" => $spl_value['section_allowed_for_hosting']);										
							}
							else
							{
								$special_section = '';
	...
	{	
		...
		if($template_details[0]['pagetype']=='1' && $template_details[0]['menuid']!=10000){
		...
		elseif($template_details[0]['pagetype']=='2' && $template_details[0]['menuid']!=10000){
	...
	public function fill_widget($widget_location ,$content)
	{
		...
		$pass_widget_instance		= (isset($content['widget_values']['data-clonedinstanceid']) && $content['widget_values']['data-clonedinstanceid'] != "") ? $content['widget_values']['data-widgetinstanceid'] = $content['widget_values']['data-clonedinstanceid'] : $content['widget_values']['data-widgetinstanceid'];
		...
		if(count($widget_instance_details)>0){
				/* if( ($widget_instance_details['publish_start_date']=='' && ((strtotime('now') > strtotime($widget_instance_details['publish_start_date'])) )) && ((($widget_instance_details['publish_end_date']=='' && (strtotime('now') < strtotime($widget_instance_details['publish_end_date']))))) && $widget_instance_details['status'] == 1 ){
	...
	public function save_html_template($menuid , $pagetype)
	{
		...
		if($xml != '' && !empty($xml))
		{
		   ...
		   $viewpage = $this->load->view("admin/view_frontend", $data, true);
		    if((strtolower(@$section_details['Sectionname'])=="home") || (@$section_details['Sectionname'] =="முகப்பு")){ $ext = ".php"; }else{  $ext = ".txt"; }
			$section_name = $section_details['Section_id'].$ext;
			$file = FCPATH.'application/views/view_template'.'/'.$section_name;
	...
	{				
		...
		if($xml != '' && !empty($xml))
		{
			...
			$sectionname             = strtolower($section_details['Sectionname']);
			$parent_sectionname      = '';
			if($section_details['ParentSectionID']!=''){
			$parent_section          = $this->widget_model->get_parent_sectionmane($section_id, "adminview");
			$parent_sectionname      = strtolower($parent_section['Sectionname']);
			}
...
<div class=" ajaxStatus ajaxFailed noDisplay" style="display: none;">Failed to load the content...</div></div>';
	...
	public function save_widget_config($is_from_copy_widget)
	{
		...
		$configuration 		= json_decode( $this->input->post('widget_configuration') );		
		...
		//$widget_publishon = date_format(date_create(@$configuration->widgetpublishOn) , 'Y-m-d H:i:s');
		//$widget_publishoff = date_format(date_create(@$configuration->widgetpublishOff) , 'Y-m-d H:i:s');
		...
		//exit;
		//echo "simple count: ".count($configuration->simpleTab->categoryList);
		//exit;
		...
		if($widget_type == 1)
		{
			...
			//$normal_tab_form_data['widget_mainsection_id'] 	= $widget_mainsection_id;
		...
		else if($widget_type == 2)
		{
			...
			if(count($simpletab_list) > 0)
			{
				...
				{					
					...
					//$simpletab_data['widget_type'] 		= $widget_type;
		...
		else if($widget_type == 3)
		{
			...
			if(count($simpletab_list) > 0)
			{
				...
				{					
					...
					$simpletab_data['Instanceid'] 		= $widget_instance_id;
					$simpletab_data['created_by'] 		= $login_user_id;
					//print_r($value->childCategory); exit;
					$nested_sub_inc = 1;
					$simpletab_sub_data = "";
					...
					$simple_tab_form_data[] = $simpletab_data;
					//print_r($simple_tab_form_data); exit;
					$nested_inc ++;
				}
			}
			//print_r($simple_tab_form_data); exit;
		...
		//$insert_data['exist'] 	= $widget_mainsection_id;
		//print_r($widget_subconfig_id_extra); exit;
				
		
			//////  Old code - before introducing temporary instance config  //////
			//$configuration_result 	= $this->template_design_model->insert_widget_configuration($insert_data, $widget_mainsection_id_extra, $widget_subconfig_id_extra, $widget_type, $version_id);
...
//echo $this->db->last_query(); exit;
	...
	public function lock_widget_articles()
	{
		...
		$user_id			= USERID;
		$return_result	= array("msg"=>"","res_status"=>0,"show_msg"=>"0", "passed_params"=>"Instance Id:".$widgetinstance_id, "msg_type"=>2, "articlelock_status"=>"", "locked_user_id"=>"", "locked_user_name"=>"");
	...
	{				
		...
		$instanceId 					= $this->template_design_model->copy_WidgetInstance($widget_instance, $reference_widget_instance); 		
