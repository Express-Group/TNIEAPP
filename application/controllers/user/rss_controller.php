...
class Rss_controller extends CI_Controller {
	   ...
	   if(count($section_id)>0)
	   {
			...
			if($section_id['Section_id']== 422){
				if($this->input->get('type')=='gallery'){
					...
					$data['rss_article']  = $this->live_db->query("SELECT a.content_id, a.summary_html, a.first_image_path, a.first_image_title, a.first_image_alt, a.title, a.url, a.tags, a.agency_name, a.author_name, a.last_updated_on FROM gallery AS a WHERE a.status='P' AND a.publish_start_date < NOW() ORDER BY a.publish_start_date DESC LIMIT 50")->result_array();
				}else if($this->input->get('type')=='video'){
					$data['Content_type']  =  $data['content_type']   = 4;
					$data['rss_article']  = $this->live_db->query("SELECT a.content_id, a.summary_html, a.video_script, a.video_image_path, a.video_image_title, a.title, a.url, a.tags, a.agency_name, a.author_name, a.last_updated_on FROM video AS a  WHERE a.status='P' AND a.publish_start_date < NOW() ORDER BY a.publish_start_date DESC LIMIT 50")->result_array();
				}else{
					
					$data['rss_article']  = $this->live_db->query("SELECT a.content_id,a.publish_start_date, a.summary_html, a.article_page_content_html as articlestory, a.article_page_image_path, a.article_page_image_title, a.title, a.url, a.tags, a.agency_name, a.author_name, a.last_updated_on FROM article AS a WHERE a.status='P' AND a.publish_start_date < NOW() ORDER BY a.publish_start_date DESC LIMIT 1000")->result_array();
				}
			}else if($section_id['Section_id']== 442){
				$year = date('Y') -1 ;
				$table = 'article_'.$year;
				if($this->archive_db->table_exists($table)){
					$query = $this->archive_db->query("SELECT a.content_id,a.publish_start_date, a.summary_html, a.article_page_content_html as articlestory, a.article_page_image_path, a.article_page_image_title, a.title, a.url, a.tags, a.agency_name, a.author_name, a.last_updated_on FROM ".$table." AS a WHERE a.status='P' AND  a.section_id='".$section_id['Section_id']."' AND a.publish_start_date < NOW() ORDER BY a.publish_start_date DESC LIMIT 1000")->result_array();
				}
				$query1 = $this->live_db->query("SELECT a.content_id,a.publish_start_date, a.summary_html, a.article_page_content_html as articlestory, a.article_page_image_path, a.article_page_image_title, a.title, a.url, a.tags, a.agency_name, a.author_name, a.last_updated_on FROM article AS a WHERE a.status='P' AND  a.section_id='".$section_id['Section_id']."' AND a.publish_start_date < NOW() ORDER BY a.publish_start_date DESC LIMIT 1000")->result_array();
				$data['rss_article']  = array_merge($query , $query1);
			}else{
	...
	public function editors_pick() {
		$this->live_db->select("content_id,content_type,CustomTitle,CustomSummary,image_path");	
		$this->live_db->from("sectionwidgetarticle");
		$this->live_db->where("widget_type",2);	
		$this->live_db->order_by("DisplayOrder","asc");
		$Result=$this->live_db->get();
		$Result = $Result->result();
		$data['new_articles'] 		= $Result; 
		$data['xml_type']			= "editorial_pick";
		echo $this->load->view("admin/editorpick_sitemap",$data,true); 
	}
	
	public function latest_sitemap(){
		$Response ='';
		$GetArticle = $this->live_db->query("SELECT content_id, title, summary_html, article_page_content_html, publish_start_date, tags, last_updated_on, url, article_page_image_path, article_page_image_alt FROM article WHERE publish_start_date < NOW() AND status='P' AND section_id!=363 ORDER BY last_updated_on DESC LIMIT 10");
		if($GetArticle->num_rows()!=0){
			foreach($GetArticle->result_array() as $ArticleDetails):
				...
				$EncodedContent 	.='var _comscore = _comscore || [];
										  _comscore.push({ c1: "2",	c2: "1234567", options: {
											...
											</script><noscript><img src="https://sb.scorecardresearch.com/p?c1=2&c2=1234567&cv=3.1&cj=1&comscorekw=fbia"></noscript></iframe></figure>';
	...
	public function latest_feed(){
		...
		if($this->input->get('category')!='' && is_numeric($this->input->get('category'))){
			...
			$SectionIds = array(170,171,172,173,174,175,177,178,179,340,341,342,343,362,180,181,182,183,184,185);
			$limit = 1;
		...
		for($i=0;$i<count($SectionIds);$i++):
			...
			foreach($GetArticle as $ArticleDetails):
				...
				//$search        = array('&', '&#39;');
				//$replace       = array('&amp;', "'");
				$search        = array('&', '&#39;','&amp;','&nbsp;','nbsp;','<br>','</br>','<br />','quot;','#39;','ldquo;','rdquo;');
				$replace       = array('&amp;', "'",' ',' ',' ','','','');
				$title         = strip_tags(str_replace($search, $replace , $title)); 
				$title			= preg_replace("|&([^;]+?)[\s<&]|","&amp;$1 ",$title);
				$publish_date_custom = new DateTime(@$ArticleDetails['publish_start_date']);
				$publish_updated_date_custom = new DateTime(@$ArticleDetails['last_updated_on']);
				$search1        = array('&', '&#39;','&amp;','&nbsp;','nbsp;','<br>','</br>','<br />','quot;','#39;','ldquo;','rdquo;');
				$replace1       = array('&amp;', "'",' ',' ',' ','','','');
				$content         	= str_replace($search1, $replace1 , $ArticleDetails['article_page_content_html']); 
				$content = preg_replace('/(<[^>]+) style=".*?"/i', '$1', $content);
	...
	public function magzterfeed(){
		...
		if($sectionId=='article' || $sectionId=='gallery' || $sectionId=='video'){
			$sectionDetails  =  $this->widget_model->get_sectionDetails(169, "live");
		}else{
