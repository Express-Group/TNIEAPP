	...
	public function get_product(){
		...
		if(count($data) > 0){
			foreach($data as $product){
				$template .='<tr class="product-'.$product['pid'].'" data-title="'.strip_tags($product['product_name']).'" data-imagepath="'.(($product['image_path']!='') ? base_url(CONTENT_IMAGE_PATH.$product['image_path']) : '').'">';
				$template .='<td>'.strip_tags($product['product_name']).'</td>';
				$template .='<td class="text-center">';
				if($product['image_path']!=''){
				$template .='<img src="'.base_url(CONTENT_IMAGE_PATH.$product['image_path']).'" style="width:75px;border-radius:5px;">';	
				}else{
				$template .='-';	
				}
				$template .='</td>';
